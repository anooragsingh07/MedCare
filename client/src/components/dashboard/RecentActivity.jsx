  ...
  return (
    <Card title="Recent activity" subtitle="Latest patients, appointments, and bills">
      {loading ? (
        ...
        <p className="text-sm text-slate-500">No activity yet. Add patients, appointments, or bills to populate this feed.</p>
      ) : (
          ...
          {items.map((row) => {
            ...
            return (
              <li key={row.id} className="flex gap-3 py-3 first:pt-0">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-hospital-50 text-hospital-700 ring-1 ring-hospital-100">
                ...
                <time className="shrink-0 text-xs font-medium text-slate-400" dateTime={row.at.toISOString()}>
