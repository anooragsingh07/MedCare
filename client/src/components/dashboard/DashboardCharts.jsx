  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex h-72 items-center justify-center">
          {appointmentPie.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={appointmentPie} dataKey="value" nameKey="name" innerRadius={52} outerRadius={88} paddingAngle={2}>
      ...
      <div className="rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm lg:col-span-2">
        ...
        <div className="mt-4 h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={patientVolume} margin={{ top: 10, right: 12, left: -8, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
              <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 11 }} axisLine={false} tickLine={false} />
